...
1. 사용자가 "오늘의 루틴 추천받기" 클릭
        ↓
2. 오늘의 컨디션 미입력 확인
   → 컨디션 점수(1~5), 근육통 부위 멀티셀렉트 화면 표시
   → 입력 완료 시 session_state["condition_{오늘날짜}"]에 저장
     (날짜가 바뀌면 자동으로 새로 입력 요구 — 별도 삭제 로직 불필요)
        ↓
3. 체력 수준 승급 자동 감지
   → 완료 루틴 10회 이상 + 레벨업 운동 3가지 이상이면
     "intermediate로 올라갈 때가 됐어요!" 배너 표시
        ↓
4. 최근 2일 훈련 부위 분석 (_get_recent_body_parts)
   → workout_logs에서 운동명을 읽어 regex로 가슴/등/하체 등 판별
        ↓
5. app.py → api_client.py → GET /db/routines/today/{user_id}
   "혹시 오늘 루틴 이미 만들어 뒀어?" 먼저 확인 (캐시 확인)
        ↓
   [이미 있으면] 저장된 루틴 그대로 보여주고 끝
   [없으면] 아래 과정 진행
        ↓
6. GET /db/logs/recent-exercises/{user_id}
   최근 7일 운동 기록 가져오기
        ↓
7. GET /db/logs/progression/{user_id}
   각 운동의 "레벨업 여부" 분석
   (3세트 × 11회 이상 달성했으면 → 무게 7.5% 올리기 제안)
        ↓
8. GET /gym/retrieve/{user_id}
   ChromaDB에서 내 헬스장 기구 정보 꺼내오기
        ↓
9. _build_routine_prompt()가 프롬프트 조립:
   - 사용자 프로필 (나이/목표/체력수준)
   - [부상 주의사항] 무릎·허리 등 injury_tags 기반 금지·대체 운동
   - [오늘 볼륨 조정] 컨디션 3점 이하면 "볼륨 20% 감소"
   - [오늘 제외할 부위] 근육통 부위 + 최근 2일 훈련 부위 합산
   - [볼륨 가이드라인] 목표별 세트·반복 수 기준
   - 점진적 향상 분석 결과 + 헬스장 기구 정보
        ↓
10. OpenAI GPT-4o-mini가 루틴 JSON 생성
        ↓
11. POST /db/routines 로 MySQL에 저장
        ↓
12. 웹 화면에 카드·차트 형태로 표시
```

> **비유:** 주치의가 "이 환자 오늘 몸 상태"·"부상 이력"·"이 병원 보유 장비 목록"을 모두 보고 "오늘은 무릎 운동 빼고, 볼륨 줄여서 상체 위주로 처방전을 쓰는 과정입니다.
...
> **현재 구현 상태 참고:** 현재 코드는 `collection.get()`으로 사용자 기구 정보를 **전체 조회**합니다. 진짜 RAG처럼 "필요한 것만 골라서 주입"하는 방식은 아직 구현되지 않았습니다. 향후 **방법 1 (운동 지식 DB RAG)** 구현 시 `collection.query(query_embeddings=...)`를 사용해 부상·목표·훈련부위 조건으로 관련 운동 가이드만 선택적으로 주입할 계획입니다.
...
| 파일 | 한 줄 설명 | 이 기능 고치려면? |
|------|-----------|----------------|
| [app.py](07_FitStep_Web/app.py) | 전체 웹 화면 (로그인·메뉴·루틴추천·기록·대시보드·기구등록·프로필수정) | 화면 디자인, UI 로직 수정 시 |
...
사용자 프로필 (나이/몸무게/목표/주의사항)
  + [부상 주의사항] injury_tags 기반 금지·대체 운동
  + [오늘 볼륨 조정] 컨디션 점수 기반 볼륨 증감 지시
  + [오늘 제외할 부위] 근육통 부위 + 최근 2일 훈련 부위 합산
  + [볼륨 가이드라인] 목표별 세트·반복 수·강도 기준
  + 최근 7일 운동 이력
  + 점진적 향상 분석 결과
  + ChromaDB에서 검색한 헬스장 기구 정보
          ↓
_build_routine_prompt() → 프롬프트 생성
          ↓
client.chat.completions.create(
    model="gpt-4o-mini",
    response_format={"type": "json_object"},
